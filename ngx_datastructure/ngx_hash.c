...
/*
 * ngx_align是对sizeof(void*)进行对齐
 * NGX_HASH_ELT_SIZE宏的值就是一个ngx_hash表中ngx_hash_elt_t实际的内存大小
 * 1. 第一个sizeof(void*)是ngx_hash_elt_t结构体重的void *value占用的内存
 * 2. +2是ngx_hash_elt_t结构体重u_short len占用的内存，2个字节
 * 3. name->key.len的原因是，ngx在对ngx_hash_elt_t进行内存复制的时候，会在他的后面连续的地址空间
 *	  复制key字符串的内容。即ngx_hash_elt_t结构体中u_char name[1]之后会跟着len-1个字节
 */
